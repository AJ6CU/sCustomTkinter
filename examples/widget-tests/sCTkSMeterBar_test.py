...
class sCTkSMeterBar(ctk.CTkFrame, ThemeableWidget):
    def __init__(self, master=None, swr_max_value=5.0, swr_visible=True, pwr_visible=True, hide_lower_row=False, width=320, height=110, **kw):
        # 1. Initialize our Themeable mixin tracker cleanly
        ThemeableWidget.__init__(self, kw)
        self._local_defaults = dict(self.final_kw)
        self._custom_disabled_map = dict(self._widget_disabled_map)

        # 2. Extract the core adaptive backplane color configuration safely
        ...
        for led_custom_key in ["fg_color", "text_color", "alarm_color", "led_on_color", "led_off_color", "font"]:
    ...
    def _draw_meter(self):
        ...
        # Resolve look variables out of local theme default dictionary snapshots safely
        bg_color = self._resolve_color(self._local_defaults.get("fg_color", ("#F8FAFC", "#0A0A0A")))
        amber_color = self._resolve_color(self._local_defaults.get("text_color", ("#FF9100", "#FF9100")))
        red_color = self._resolve_color(self._local_defaults.get("alarm_color", ("#FF2200", "#FF2200")))
        led_on_color = self._resolve_color(self._local_defaults.get("led_on_color", ("#10B981", "#10B981")))
        led_off_color = self._resolve_color(self._local_defaults.get("led_off_color", ("#E2E8F0", "#1F2937")))
        disabled_color = self._resolve_color(("#94A3B8", "#334155"))
        self.canvas.configure(bg=bg_color)
        ...
        for pct, label_str in bar_scale_mappings:
            ...
            if label_str: self.canvas.create_text(tx, sig_y - 14, text=label_str, fill=color, font=("Arial", 9, "bold"), anchor="center")

        self.canvas.create_text(start_x, sig_y - 14, text="S", fill=amber_color, font=("Arial", 9, "bold"), anchor="center")
        self.canvas.create_text(start_x + (total_length * 0.5), sig_y + 6, text="SIG", fill=amber_color, font=("Arial", 10, "bold"), anchor="n")
        ...
        self.canvas.create_text(start_x + (total_length * ((mid_gap_start / num_led_segments) * 0.5)), lower_y - 20, text="SWR", fill=swr_label_color, font=("Arial", 10, "bold"), anchor="n")
        self.canvas.create_text(start_x + (total_length * ((mid_gap_end / num_led_segments) + ((1.0 - (mid_gap_end / num_led_segments)) * 0.5))), lower_y - 20, text="PWR", fill=pwr_label_color, font=("Arial", 10, "bold"), anchor="n")
        ...
        for val, label in [(0, "0"), (50, "50"), (100, "100%")]:
            ...
            self.canvas.create_line(tx, lower_y, tx, lower_y + 6, fill=color, width=1)
            self.canvas.create_text(tx - 4 if val == 100 else tx, lower_y + 12, text=label, fill=color, font=("Arial", 9, "bold"), anchor="n")
