...
A custom, theme-compliant dominant messaging and logging area widget.
...
UI source file: sCTkTextboxPrimary.ui (Integrated via programmatic composition)
"""
import os
import sys
import platform
...
class sCTkTextboxPrimary(ctk.CTkTextbox, ThemeableWidget):
    def __init__(self, master=None, **kw):
        print("\n[Forensic Check] PURE PROGRAMMATIC TEXTBOX PIPELINE ACTIVE NOW!\n")

        # 1. Fire our shared theme logic first. It automatically finds "sCTkTextboxPrimary" in themes.json
        ...
        # 3. Initialize CustomTkinter natively as a pure programmatic textbox asset
    ...
    def configure(self, *args, **kwargs):
        ...
        # We cache the requested state mode inside instance memory and pass it straight down
        # to the unified look updater to ensure the read-only flag tracks your changes flawlessly!
        if "state" in kwargs:
    ...
    def _update_current_visual_state(self):
        """MASTER VISUAL ROUTER FIXED: Enforces strict sequential order of operations."""
        ...
        # 2. SEQUENTIAL LOCK PASS:
        # We execute the native state flag configuration at the absolute end of the update tree.
        # This completely seals the text editor loop, forcing a perfect input lockout!
        if is_disabled:
...
if __name__ == "__main__":
    ...
    btn_toggle = ctk.CTkButton(base, text="Lock Logger Feed", command=toggle_logger_states)
