...
if __name__ == "__main__":
    ...
    initial_val = 0.45
    lbl_status = sCTkLabelSecondary(base, text=f"Telemetry Progress Output: {int(initial_val * 100)}% / 100%")
    ...
    def step_progress():
        if widget.get_state() == "disabled": return
        next_val = widget.get() + 0.05
        if next_val > 1.0: next_val = 0.0
        widget.set(next_val)
