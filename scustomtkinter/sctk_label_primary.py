...
A theme-compliant, high-emphasis header label widget -- the most prominent of
the library's three label tiers (see also sCTkLabelSecondary,
sCTkLabelTertiary). Inherits directly from ctk.CTkLabel so CustomTkinter
...
Base class order matters here: `class sCTkLabelPrimary(ctk.CTkLabel,
...
place, so there's no equivalent of the click-passthrough bug found and fixed
on the button widgets.

Internal state attribute is named self._custom_current_state, matching the
convention used across the rest of the library (sCTkComboBox, the button
family, sCTkCheckBox, etc.) -- an earlier version of this file used
self._current_state instead, which was inconsistent both within the label
...
class sCTkLabelPrimary(ctk.CTkLabel, ThemeableWidget):
    """Themeable, high-emphasis header label.
    ...
    Adds to native ctk.CTkLabel:
      ...
      - Pygubu Designer property introspection for `state`, `fg_color`, and
        `text_color` via a single-argument configure() call, and a cget()
    ...
    def __init__(self, master: Optional[Any] = None, **kwargs: Any) -> None:
        ...
        Args:
            ...
            **kwargs: `state` is pulled out explicitly below (applied after
                ...
                (see the "sCTkLabelPrimary" block in sCTkThemes.json).
        """
        ...
        # Remove any stray "state" key from final_kw -- it was already
        # extracted above, but this guards against it being reintroduced by
        # a theme override with the same name.
    ...
    def configure(self, *args: Any, **kwargs: Any) -> Any:
        ...
        # below never matched anything; every single-argument call silently
        # fell through to native configure() instead. Don't reintroduce that.
        if len(args) == 1:
