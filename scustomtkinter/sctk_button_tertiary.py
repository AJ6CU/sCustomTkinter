...
A theme-compliant tertiary (outline-style) action button widget -- the
...
Base class order matters here: `class sCTkButtonTertiary(ctk.CTkButton,
...
class sCTkButtonTertiary(ctk.CTkButton, ThemeableWidget):
    """Themeable tertiary (outline-style) action button.
    ...
    This button style has a transparent fg_color by design (it's an outline
    button: border + text, no filled background). The theme's disabled_map has
    no fg_color or hover_color entries at all, which is deliberate -- omitting
    them means those properties simply keep their current ("transparent")
    value when disabled, rather than gaining a solid disabled-fill background
    that a filled button would want but an outline button shouldn't.
    Similarly, pressed_map has no hover_color entry; _update_current_visual_state()
    explicitly falls back to the normal-state hover_color in that case (see
    that method) rather than leaving it unset.
    ...
    def _active_theme_map(self):
        """
        The map a repaint currently reads, following this button's precedence:
        disabled > pressed > normal.

        The base version knows only about disabled and normal, so an override
        made while the button was pressed would land in the normal map and not
        show until it was released.
        """
        if getattr(self, "_custom_current_state", "normal") == "disabled":
    ...
    def _update_current_visual_state(self) -> None:
        ...
        The pressed branch explicitly falls back to the normal-state
        hover_color if pressed_map doesn't define one -- it doesn't, by
        design, for this widget's theme block. See this class's docstring.
        ...
        if getattr(self, "is_pressed", False):
            ...
            # pressed_map has no hover_color entry for this widget by design
            # (see class docstring) -- fall back to the normal-state value
            # rather than leaving hover_color unset while pressed.
