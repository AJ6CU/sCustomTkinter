...
A theme-compliant, mid-emphasis interface label widget -- sits between
sCTkLabelPrimary and sCTkLabelTertiary in the library's three label tiers.
Inherits directly from ctk.CTkLabel so CustomTkinter handles native rendering;
this class layers automatic light/dark theme resolution and a distinct
enabled/disabled visual state on top.

Base class order matters here: `class sCTkLabelSecondary(ctk.CTkLabel,
...
class sCTkLabelSecondary(ctk.CTkLabel, ThemeableWidget):
    """Themeable, mid-emphasis interface label.
    ...
    def __init__(self, master: Optional[Any] = None, **kwargs: Any) -> None:
        ...
        Args:
            ...
            **kwargs: `state` is pulled out explicitly below (applied after
                ...
                (see the "sCTkLabelSecondary" block in sCTkThemes.json).
        """
    ...
    def _update_current_visual_state(self) -> None:
        ...
        it's missing, this raises immediately rather than silently omitting
        the property, which is what an earlier version of this method did (no
        fallback and no error -- text_color would simply never get set). Per
        this project's design, ThemeableWidget-based widgets are meant to fail
        hard on incomplete theme data, not silently do nothing.
        """
